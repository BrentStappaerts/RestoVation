...
class Tables extends CI_Controller {
	...
	function editTable($id) {
        ...
        $this->form_validation->set_rules('number', 'Nummer tafel', 'required');
		$this->form_validation->set_rules('amount', 'Aantal', 'required');
		...
		{		
			...
			if ($this->form_validation->run() === true)
			{
				...
				redirect(base_url().'index.php/tables/edittable'.$id);
			}			
		}
		...
		$this->data['number'] = array(
			...
			'cols'		=>	60,
			'rows'		=>	5,
			'value' 	=> $this->form_validation->set_value('tafelnummer', $table['tafelnummer']),
		);
		...
		$this->load->view('edit_table', $this->data);
	}	
	
	function deleteTable($id) {
		$this->table_model->delTable($id);
		
		$this->session->set_flashdata('message', '<p>Tafels zijn verwijderd</p>');
		
		redirect('index.php/tables/deletetable');
	}
}